...
class BoardState {
    ...
    /**
     * Strokes written with the text pen that have not been converted yet.
     *
     * Collected until the pen has been still for a moment, so a word written
     * in several strokes converts as ONE word — recognising each stroke as it
     * lands would read "cat" as three separate letters.
     *
     * Plain list, not state: read only by the conversion coroutine.
     */
    val pendingTextStrokes: MutableList<Stroke> = mutableListOf()

    /** True while a conversion is in flight, so the board can say so. */
    var isConvertingText by mutableStateOf(false)

    /**
     * The box the last conversion produced, so the next one can continue it.
     *
     * A teacher writing "hello" who pauses after the "h" gets two conversions,
     * and without this they get two separate boxes. Cleared whenever the pen
     * moves elsewhere, so a genuinely new word still starts a new box.
     */
    /**
     * The text box currently open for editing, if any.
     *
     * Lives here rather than inside the text layer because the CANVAS has to
     * be able to close it: tapping the board is the commit gesture, and the
     * canvas consumes that tap before any text composable sees it.
     */
    var editingTextBoxId by mutableStateOf<String?>(null)

    /**
     * Set while a stroke is being generated by an instrument rather than drawn.
     *
     * Instrument-made geometry is exact by construction, so running it through
     * shape recognition can only degrade it — a compass arc snapped to the
     * recogniser's circle no longer has the radius the teacher measured.
     */
    var suppressShapeSnap: Boolean = false

    var lastTextBoxId: String? = null

    /**
     * Right edge of the ink that produced [lastTextBoxId], in world units.
     *
     * Kept separately because a box's width is the point its text WRAPS at,
     * which sits well past the text itself — measuring the gap from there
     * would treat a new word half a screen away as a continuation.
     */
